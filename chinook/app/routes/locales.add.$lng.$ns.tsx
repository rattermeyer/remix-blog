...
export const action = async ({request, params}: ActionFunctionArgs) => {
    ...
    for (const key of Object.keys(parsedBody)) {
        await db.insert(missing_translationsInChinook).values({
            language: lng ?? 'unknown',
            namespace: ns ?? 'unknown',
            key: key,
            default_value: parsedBody[key],
        }).onConflictDoUpdate({
            target: [missing_translationsInChinook.language, missing_translationsInChinook.namespace, missing_translationsInChinook.key],
            set: {
